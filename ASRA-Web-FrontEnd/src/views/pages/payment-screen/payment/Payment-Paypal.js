            ...
            CreatePayment(resRequestBody).then((res) => {
              ...
              paymentBillForRoom(resData).then((res) => {
                socket.emit("sendNotification", {
                  roomId: state?.notificationData?.roomId,
                  senderId: state?.notificationData?.senderId,
                  contractId: state?.notificationData?.contractId,
                  senderName: state?.notificationData?.senderName,
                  receiverName: state?.notificationData?.receiverName,
                  message: state?.notificationData?.message,
                  type: state?.notificationData?.type,
                  thumbnail: state?.notificationData?.thumbnail,
                });
                openNotification(SUCCESS, res.message);
                navigate(`/payment/information/${billId}`, {
                  state: {
                    orderId: order.id,
                  },
                });
